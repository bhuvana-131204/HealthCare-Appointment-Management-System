...
/**
 * ConsultationController is a REST controller that handles HTTP requests for managing consultation records.
 * It provides end points for creating, retrieving, updating, and deleting consultation records.
 * 
 * @RestController - Indicates that this class is a REST controller.
 * @RequestMapping("/consultation") - Maps HTTP requests to /consultation to this controller.
...
public class ConsultationController {
     ...
     * @return ResponseEntity<Response<?>> - The response entity containing the created consultation details or an error message.
     */
     ...
     * 
     * @return ResponseEntity<Response<?>> - The response entity containing the list of all consultation records or an error message.
     */
    ...
    public ResponseEntity<Response<?>> getConsultationDetails() {
        try {
            ...
            List<ConsultationDto> consultationList = consultantService.getAllConsultationDetails().stream()
     ...
     * @return ResponseEntity<Response<?>> - The response entity containing the list of consultation records filtered by Patient ID or an error message.
     */
    
    @GetMapping("/appointment/{appId}")
    public ResponseEntity<Response<?>> getConsultationDetailsByPatientID(@PathVariable String appId) {
        try {
            log.info("Retrieving the consultation details filtered by Patient ID..");
            List<ConsultationDto> consultationList = consultantService.findConDetailsByAppId(appId).stream()
                .map(consultation -> new ConsultationDto(
                    consultation.getConsultationId(),
                    consultation.getAppointmentId(),
                    consultation.getNotes(),
     ...
     * @param consultationDto - The consultation data transfer object containing the updated details of the consultation.
     * @return ResponseEntity<Response<?>> - The response entity containing the updated consultation details or an error message.
     */

    @PutMapping("/update/consul/{conId}")
    public ResponseEntity<Response<?>> updateConsultation(@PathVariable String conId, @RequestBody @Valid ConsultationDto consultationDto) {
        try {
    ...
    public ResponseEntity<Response<?>> removeConsultationDetails(@PathVariable String conId) {
        try {
